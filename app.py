...
def index():
    if current_user.role == "admin":
        ...
        appointments = fetch_query(
            "SELECT * FROM Appointments WHERE patient_id = ?", (current_user.id,)
        )

    return render_template(
        "index.html", patients=patients, doctors=doctors, appointments=appointments
    )
...
def user_management():
    if request.method == "POST":
        ...
        print(newrole)
        execute_query(
            "UPDATE Users SET role='{value1}' WHERE id ='{value2}'".format(
                value1=newrole, value2=role
            )
        )
        users = fetch_query("SELECT * FROM Users")
